...
const unsigned char Rcon[9] = { 0x00,0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };//定义轮常量
bool progress = true;   //true为加密过程，false为解密过程 
...
void encAES_256(int sk[8], char* massage)
{
    int test = 1;
...
void decAES_256(int sk[8])
{
    int test = 1;
    cout << "***************AES解密算法正在运行中***************" << endl;
    while (test) {
        Input(3);    //输入密文 
        unsigned char result[17];
        cout << "解密过程如下所示：" << endl;
        DeCipher(input, result);
        system("pause");
        cout << "(0)退出  (1)继续测试:  ";
        cin >> test;
    }
